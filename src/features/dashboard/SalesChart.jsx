...
const SalesChart = ({ bookings, numOfDays }) => {
  ...
  const intervalDate = eachDayOfInterval({ start: subDays(new Date(), numOfDays - 1), end: new Date() });

  const data = intervalDate.map((days) => ({
    label: format(days, 'MMM dd'),
    totalSales: bookings
      .filter((booking) => isSameDay(days, booking.created_at))
  ...
  return (
    <StyledSalesChart>
      <Heading as="h2">
        Sales From {format(intervalDate.at(0), 'dd, MMM yyyy')} &mdash; {format(intervalDate.at(-1), 'dd, MMM yyyy')}
      </Heading>
